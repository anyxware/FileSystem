...
struct TNode
{
	...
	TNode(k key, v value) : pair(key, value)
	{
		leftChild = rightChild = nullptr;
		height = 0;
	};
...
private:
	...
	TNode<k, v>* insert(TNode<k, v>* root, const k& key, const v& value)
	{
		if (root == nullptr)
		{
		...
		//left left case
		if (getBalanceFactor(root) > 1 && key < root->leftChild->pair.first)
		{
		...
		//right right case
		else if (getBalanceFactor(root) < -1 && key > root->rightChild->pair.first)
		{
			root = this->singleLeftRotation(root);
		}

		//left right case
		else if (getBalanceFactor(root) > 1 && key > root->leftChild->pair.first)
		{
	...
	void inorderPrint(TNode<k, v> const* node) const
	{
		if (node == nullptr)
			return;
	...
	v* search(TNode<k, v>* n, const k& key) const
	{
		if (n == nullptr)
			return nullptr;
	...
	TNode<k, v>* deleteKey(TNode<k, v>* root, const k& key)
	{
		...
		else //root node (the passed parameter) has the key that we need to delete
		{
			...
			else  //case 3: none of the subtrees of node to delete is nullptr.
			{
				...
				root->pair.first = newNodeToDelete->pair.first;
				root->pair.second = newNodeToDelete->pair.second;
				//root->pair = std::make_pair(newNodeToDelete->pair.first, newNodeToDelete->pair.second);

				//in this new recursive call, case 0 and 3 will never exist. Only case 1 or case 2 will exist.
				root->rightChild = deleteKey(root->rightChild, root->pair.first);
			}//end of case 3.
		}
...
public:
	...
	AVL<k, v>& operator=(AVL<k, v> const& that) {
		AVL<k, v> tmp = AVL<k, v>(that);
